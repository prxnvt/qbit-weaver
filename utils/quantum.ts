...
/** Internal version that returns ComplexArray for performance */
const createInitialStateInternal = (numQubits: number): ComplexArray => {
...
/** Public version that returns Complex[] for backward compatibility */
export const createInitialState = (numQubits: number): Complex[] => {
  return toComplexObjectArray(createInitialStateInternal(numQubits));
};
...
export const simulateCircuit = (grid: CircuitGrid): Complex[][] => {
  ...
  let currentState = createInitialStateInternal(numRows);
...
export const measureQubit = (
  ...
  return { result, probability: measuredProb, collapsedState: toComplexObjectArray(collapsedState) };
};
...
export const runCircuitWithMeasurements = (
  ...
  let currentState = createInitialStateInternal(numFilteredRows);
  ...
  for (let col = 0; col < numCols; col++) {
    ...
    for (let i = 0; i < result.measureRows.length; i++) {
      ...
      currentState = fromComplexObjectArray(collapsedState);
    }
  }
